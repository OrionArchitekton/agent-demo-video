...
describe("remote render parity", () => {
  ...
  it("fails loudly and leaves the local inputs untouched when the render host is unreachable", async () => {
    const base = mkdtempSync(join(tmpdir(), "adv-safe-"));
    const inputs = await makeInputs(base);
    const before = inputs.rawSegments.map((p) => statSync(p).size);
    ...
    const after = inputs.rawSegments.map((p) => statSync(p).size);
